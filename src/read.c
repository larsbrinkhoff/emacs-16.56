     ...
     Lisp_Object readcharfun;
{
  ...
  if (XTYPE (readcharfun) == Lisp_Buffer)
	...
	return -1;
      c = *(unsigned char *) &(inbuffer->dotloc > inbuffer->size1 ? inbuffer->p2 : inbuffer->p1)[inbuffer->dotloc];
      inbuffer->dotloc++;
      return c;
    }
...
Lisp_Object
closefile (stream)  /* used as unwind-protect function in load */
     Lisp_Object stream;
{
     ...
     register Lisp_Object readcharfun;
{
 ...
 retry:
  ...
  c = READCHAR;
  if (c < 0) Fsignal (Qend_of_file, Qnil);
  ...
  switch (c)
	...
	/* If purifying, and string starts with \ newline,
	   ...
	   that we are really going to find in etc/DOCSTRnnnn  */
...

	{
	  ...
	  while (c > 040 && 
		   ...
		   ))
	    {
	      if (p == end)
		{
		  register char *new = (char *) realloc (read_buffer, read_buffer_size *= 2);
		  p += new - read_buffer;
		  read_buffer += new - read_buffer;
		  end = read_buffer + read_buffer_size;
		}
     ...
     register Lisp_Object readcharfun;
{
  ...
  register Lisp_Object elt, val, tail;
